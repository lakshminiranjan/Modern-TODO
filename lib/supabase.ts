...
export async function updateEmailTemplates() {
  try {
    ...
    // IMPORTANT: Supabase generates the OTP and sends it in the email
    // The OTP in the email ({{ .Token }}) is different from any locally generated OTP
    // Users should enter the OTP they received in the email, not any OTP shown in logs
    if (!OTP_TEMPLATE.includes('{{ .Token }}')) {
      console.warn('WARNING: OTP template does not contain the correct placeholder: {{ .Token }}');
      console.warn('Please update the template in lib/email-templates.ts');
      
      // Check for alternative placeholders that might be used
      if (OTP_TEMPLATE.includes('{{.Token}}') || OTP_TEMPLATE.includes('{{.token}}') || 
          OTP_TEMPLATE.includes('{{ .token }}') || OTP_TEMPLATE.includes('{{.Data.otp}}') || 
          OTP_TEMPLATE.includes('{{ .Data.otp }}')) {
        console.warn('Found alternative OTP placeholder format. Please update to use {{ .Token }} exactly.');
      }
    } else {
      console.log('OTP template contains the correct placeholder: {{ .Token }}');
      console.log('NOTE: The OTP sent in the email is generated by Supabase and may differ from any locally generated OTP');
    }
...
export async function initializeSupabaseOtpSettings() {
  try {
    ...
    // Clear any stored session data and reset counters
    await AsyncStorage.removeItem('supabase_session');
    await AsyncStorage.removeItem('supabase_auth_token');
    await AsyncStorage.removeItem('last_auth_request_time');
    ...
    console.log('   - Ensure the OTP template includes {{ .Token }} (not {{ .Data.otp }} or {{ .OTP }})');
    ...
    const { OTP_TEMPLATE } = require('./email-templates');
    if (OTP_TEMPLATE && OTP_TEMPLATE.includes('{{ .Token }}')) {
      console.log('OTP template contains the correct placeholder: {{ .Token }}');
    } else {
